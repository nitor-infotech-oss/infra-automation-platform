...
#$network_switch = Get-Attr -obj $params -name network_switch -default 'New-VMSwitch -name ExternalSwitch  -NetAdapterName Ethernet -AllowManagementOS $true'
...
$imgpath = Get-Attr -obj $params -name imgpath -default $null
...
Function VM-Create {
  ...
  if (!$CheckVM) {
    ...
    if ($diskpath) {
      ...
      $results = invoke-expression "Set-VMDvdDrive -VMName '$name' -ControllerNumber 1 -Path '$imgpath'"      
    ...
    Function VM-Delete {
     ...
     #(Get-WmiObject -class Win32_TSGeneralSetting -Namespace root\cimv2\terminalservices -ComputerName WIN-K16S9QPPMFD -Filter "TerminalName='RDP-tcp'").SetUserAuthenticationRequired(0)
